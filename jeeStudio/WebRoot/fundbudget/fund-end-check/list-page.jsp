...
function activity_operator(APP_ID,mid,precursorId,gridId){
	initEasyuiWindow('activity-tab-window','资金结算审批');
	// initOpenWindow('editPageWindow_${listPageRander}','资金结算审批','editPage_${listPageRander}');
	var customPath='&customPath=/fundbudget/fund-end-check/activity-tab.jsp'
		  ...
		  columns:[[
								...
								// 判断是否使用编辑
									   ...
									   var loadEditUrl='&listFormID=${listPage.id}&listPageRander=${listPageRander}';
										...
										if((rowData.ENV_DATASTATE=='暂存')||(rowData.ENV_DATASTATE=='')||(rowData.ENV_DATASTATE==null)){
							  	 			opertorString='<img  style="cursor:hand;" title="编辑" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/images/ioc-editor.gif" onclick=loadEditPage_easyui("formengine/editPageAction.action?formId='+editFormID+'&customPath=/fundbudget/fund-end-check/edit-load.jsp","'+loadEditUrl+'","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;结算审批编辑","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
										}else{
											if(precursorId=='3'){
			...
			toolbar:[
						...
						</c:choose>
						{
							...
							iconCls:'icon-add',
							handler:function(){
								loadEditPage_easyui("formengine/editPageAction.action?formId="+editFormID+"&customPath=/fundbudget/fund-end-check/edit-load.jsp","&listPageRander=${listPageRander}","0","${idkey}",""+editPageDataDiv,"${listPage.editPage.isUseTab}",""+editWindowID,"&nbsp;结算审批录入","editPage_${listPageRander}","${listPage.parentAppId}");
							}
						},'-',
						{
							id:'btnsave',
