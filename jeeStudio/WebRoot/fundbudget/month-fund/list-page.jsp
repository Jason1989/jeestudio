						...
						,{field:'col4',title:'操作',width:100,rowspan:2,align:'center',	
							formatter:function(value,rowData,rowIndex){
									...
									<c:choose>
										...
										<c:otherwise>
										    var loadEditUrl='&listFormID=${listPage.id}&gridID=fd_formlist_table_${listPageRander}&fndMonInvestModel.invMonthId='+rowData.INV_MONTH_ID+'&startOrNextNode=start';
											if((rowData.INV_STATE=='暂存')||(rowData.INV_STATE=='变更暂存')){
								        		opertorString='<img  style="cursor:hand;" title="编辑" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/images/ioc-editor.gif" onclick=loadEditPage_easyui("'+editFormID+'","'+loadEditUrl+'","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;月度预算录入","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
											}else if(rowData.INV_STATE=='归档'){
												//todo :更新新老数据 isChangeData
										  		opertorString='<img  style="cursor:hand;" title="变更" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/jquery-easyui-1.1.2/themes/images/ico_2.jpg" onclick=loadEditPage_easyui("'+editFormID+'","'+loadEditUrl+'&isChangeData=true","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;月度预算变更","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;'+opertorString;//变更按钮
					        				}else{
										  		opertorString='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
										  	}	
										</c:otherwise>
								 ...
								 //判断是否使用查看
								   ...
								   <%
									...
									%>
								 opertorString=opertorString+'<img  style="cursor:hand;" title="查看详情" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/images/mokuai.png" onclick=loadViewPage("'+loadViewUrl+'","'+viewDataDivID+'","${viewPage.isUseTab}","'+viewWindowID+'","&nbsp;月度预算查看","viewPage_${listPageRander}"); />&nbsp;&nbsp;&nbsp;&nbsp;';
								 <c:choose>
									<c:when test="${deleteIdkey =='keynull'}"></c:when>
									  <c:otherwise>
											...
											%>
											if((rowData.INV_STATE=='暂存')||(rowData.INV_STATE=='变更暂存')){
								        		 opertorString=opertorString+'<img  style="cursor:hand;" title="删除" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/images/ioc-delete.gif"  onclick=deleteInvMonth("fbudget/zfb_deleteInvMonth.action?invMonthId='+rowData.INV_MONTH_ID+'","'+gridID+'"); />&nbsp;&nbsp;&nbsp;&nbsp;';
										  	}else{
							...
							<c:otherwise>
								{
									...
									iconCls:'icon-add',
									handler:function(){
										loadEditPage_easyui(""+editFormID,"&gridID=fd_formlist_table_${listPageRander}&startOrNextNode=start","0","${idkey}",""+editPageDataDiv,"${listPage.editPage.isUseTab}",""+editWindowID,"&nbsp;资金录入","editPage_${listPageRander}","${listPage.parentAppId}");
									}
							   },'-',	
						    </c:otherwise>
						 </c:choose>	
