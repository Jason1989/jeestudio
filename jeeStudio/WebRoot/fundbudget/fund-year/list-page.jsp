				    ...
				    <s:else>,</s:else>    
					{
						...
						,{field:'col4',title:'操作',width:100,rowspan:2,align:'center',	
							formatter:function(value,rowData,rowIndex){
							   ...
							   var loadEditUrl='&listFormID=${listPage.id}&listPageRander=${listPageRander}&YEAR_INV_ID='+rowData.YEAR_INV_ID;
								<%
								...
								%>
								if(rowData.BUDGET_STATE=='0'){
								   //暂存状态下提交
					        		opertorString='<img  style="cursor:hand;" title="编辑" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/images/ioc-editor.gif" onclick=loadEditPage_easyui("formengine/editPageAction.action?formId='+editFormID+'&customPath=/fundbudget/fund-year/edit-load.jsp","'+loadEditUrl+'","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;年度预算录入","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
								}else if(rowData.BUDGET_STATE=='1'){
								    //归档状态下 变更
									opertorString='<img  style="cursor:hand;" title="变更" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/jquery-easyui-1.1.2/themes/images/ico_2.jpg" onclick=loadEditPage_easyui("formengine/editPageAction.action?formId='+editFormID+'&customPath=/fundbudget/fund-year/fund-year-change.jsp","'+loadEditUrl+'","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;年度预算变更","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;'+opertorString;//变更按钮
					        	}else if(rowData.BUDGET_STATE=='4'){
					        	    // 变更暂存状态下  编辑
					        		opertorString='<img  style="cursor:hand;" title="编辑" sytle="cursor:pointer;" src="<%= request.getContextPath()%>/images/ioc-editor.gif" onclick=loadEditPage_easyui("formengine/editPageAction.action?formId='+editFormID+'&customPath=/fundbudget/fund-year/fund-year-change.jsp","'+loadEditUrl+'","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;年度预算变更","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;'+opertorString;//变更按钮
					        	}else{
					        	    //2 作废  3 老数据变更中  5 已变更
			...
			toolbar:[
						<c:choose>
										...
										
							{
								...
								iconCls:'icon-add',
								handler:function(){
									loadEditPage_easyui("formengine/editPageAction.action?formId="+editFormID+"&customPath=/fundbudget/fund-year/edit-load.jsp","&listPageRander=${listPageRander}","0","${idkey}",""+editPageDataDiv,"${listPage.editPage.isUseTab}",""+editWindowID,"&nbsp;年度预算录入","editPage_${listPageRander}","${listPage.parentAppId}");
