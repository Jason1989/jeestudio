	...
	return {
		...
		init: {
			...
			clock: function() {
				...
				var clock_time = clock_date+' '+weekday + ' ' + hour + ':' + minute ;
				// Shove in the HTML.
			...
			/*
				...
				$('a').live('click', function(ev) {
					...
					if ($.trim(url).match(/^#/) || $.trim(url).match(/^javascript[:]void/)) {
						ev.preventDefault();
						ev.stopPropagation();
					}
					else {
...
*/
				...
				$('div.window_desktop').live('mousedown', function() {
					// Bring window to front.
					JQD.util.window_flat();
				...
				// Double click top bar icon to close, ala Windows OS.
					...
					$(this).closest('div.window_desktop').hide();
				...
				$('#show_desktop').live('click', function() {
					...
					if ($('div.window_desktop:visible').length) {
						$('div.window_desktop').hide();
					}
					else {
		...
		util: {
			...
			window_flat: function() {
				$('div.window_desktop').removeClass('window_stack');
			},
			//
			// Resize modal window.
			//改变窗口的大小
			...
			window_resize: function(el) {
				...
				var win = $(el).closest('div.window_desktop');
