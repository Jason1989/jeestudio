function createEditPageForm(dataObjId){
	/**
	 * 构建列表页XML整体
	 */
	var xmlUtils = new XmlUtils({dataType:'json'}); 
	var formType = 'editPage';
	createListPageXml(xmlUtils,formType,null,null);
	
	$('#fm_formadd_form').form('submit',{ 
		url:'form/form!addForAll.action?dataObjectId='+dataObjId+'&form.type=editPage', 
		onSubmit:function(){ 
		...
		success:function(data){ 
		    if("exist" == data) {						
            	...
            	$("#fm_form_add").window({'href':'form/form!toAddForALL.action?dataObjectId='+dataObjId+'&pageType=viewPage&editPageId='+responseObject.editPageId+'&random='+parseInt(10*Math.random())});		
