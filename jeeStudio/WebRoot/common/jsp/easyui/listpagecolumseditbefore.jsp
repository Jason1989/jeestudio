...
<%
	...
	Map authority = (Map) request.getSession()
			.getAttribute("authority");
	ListPage listPage = (ListPage) request.getAttribute("listPage");
	String workflowFiter = (String) request
			.getAttribute("workflowFiter");
%>
<!-- 构件内容 -->
<c:choose>
	<c:when test="${listPage.isShowOperator eq 'true'}">
			...
			<c:otherwise>
									   var loadEditUrl='&listFormID=${listPage.id}&listPageRander=${listPageRander}';
										<%
				EditPage editPage = ((ListPage) request
										.getAttribute("listPage")).getEditPage();
								if (editPage != null) {
									if (editPage.getEditPageParams() != null) {
										Param param = null;
										for (int i = 0; i < editPage.getEditPageParams().size(); i++) {
											param = (Param) editPage
													.getEditPageParams().get(i);
											out
													.println(" loadEditUrl=loadEditUrl+'&"
															+ param.getKey()
															+ "='+rowData."
															+ param.getKey() + ";");
			...
			%>
				...
				<%
					if (authority != null) {
										if (authority.get("'" + listPage.getId()
												+ "_edit'") != null) {
				%>
			...
			<c:otherwise>
									   var loadCopyUrl='&listFormID=${listPage.id}';
										<%
				CopyPage copyPage = ((ListPage) request
										.getAttribute("listPage")).getCopyPage();
								EditPage editPage1 = ((ListPage) request
										.getAttribute("listPage")).getEditPage();
								if (editPage1 != null) {
									if (editPage1.getEditPageParams() != null) {
										Param param = null;
										for (int i = 0; i < editPage1
												.getEditPageParams().size(); i++) {
											param = (Param) editPage1
													.getEditPageParams().get(i);
											out
													.println(" loadCopyUrl=loadCopyUrl+'&method=copy&"
															+ param.getKey()
															+ "='+rowData."
															+ param.getKey() + ";");
										}
				...
				<%
					if (authority != null) {
										if (authority.get("'" + listPage.getId()
												+ "_copy'") != null) {
				%>
											opertorString+='<img style="cursor: hand;" title="复制"
					sytle="cursor:pointer;"
					src="<%=request.getContextPath()%>/images/ioc-copy.png"
					onclick=loadEditPage_easyui( "${listPage.id}","${isAbleWorkFlow}","'+copyFormID+'","'+loadCopyUrl+'","1","${idkey}","'+copyPageDataDiv+'","false","'+copyWindowID+'","&nbsp;${copyPage.editPage.title}","copyPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
										<%
					}
									} else {
				%>
			...
			<c:otherwise>
									   ...
									   <%
				ViewPage viewPage = ((ListPage) request
										.getAttribute("listPage")).getViewPage();
								if (viewPage != null) {
									if (viewPage.getViewPageParams() != null) {
										Param param = null;
										for (int i = 0; i < viewPage
												.getViewPageParams().size(); i++) {
											param = (Param) viewPage
													.getViewPageParams().get(i);
											out
													.println(" loadViewUrl=loadViewUrl+'&"
															+ param.getKey()
															+ "='+rowData."
															+ param.getKey() + ";");
										}
				...
				<%
					if (authority != null) {
										if (authority.get("'" + listPage.getId()
												+ "_view'") != null) {
				%>
										opertorString=opertorString+'<img style="cursor: hand;"
					title="查看详情" sytle="cursor:pointer;"
					src="<%=request.getContextPath()%>/images/mokuai.png"
					onclick=loadViewPage( "'+loadViewUrl+'","'+viewDataDivID+'","${viewPage.isUseTab}","'+viewWindowID+'","&nbsp;${viewPage.title}","viewPage_${listPageRander}"); />&nbsp;&nbsp;&nbsp;&nbsp;';
									<%
					}
									} else {
				%>
			...
			<c:otherwise>
										...
										<%
				String deleteIDFields = pageContext.getAttribute(
										"deleteIdkey").toString();
								String[] deleteKeyArray = deleteIDFields.split(",");
								for (int i = 0; i < deleteKeyArray.length; i++) {
									if ("".equals(deleteKeyArray[i])
											|| (deleteKeyArray[i] == null)) {

									} else {
										out.println(" deleteUrl=deleteUrl+'&"
												+ deleteKeyArray[i] + "='+rowData."
												+ deleteKeyArray[i] + ";");
									}
								}
			%>
				...
				%>
									    opertorString=opertorString+'<img style="cursor: hand;"
					title="删除" sytle="cursor:pointer;"
					src="<%=request.getContextPath()%>/images/ioc-delete.gif"
					onclick=deleteData_easyui( "'+deleteUrl+'","'+gridID+'"); />&nbsp;&nbsp;&nbsp;&nbsp;';
									<%
					}
				...
				<c:when test="${isshow eq '1' && buttonrules ne ''}">
											 ...
											 if('jiekou' == buttonrules.isJS){
											 	  if('editPage' == buttonrules.tabspagetype){
											 	  	  ...
											 	  	  opertorString += '<img style="cursor: hand;"
						title="${buttonname}" sytle="cursor:pointer;"
						src="<%=request.getContextPath()%>/${buttonicon}"
						onclick=createSavebutt( "${isAbleWorkFlow}","<%=Constant.WORKFLOW_ENABLE%>","${listPageRander}","${listPage.id}","'+buttonrules.tabspageurl+'","'+loadEditUrl_JS+'","0","${idkey}","'+editPageDataDiv_zdy+'","${editPage.isUseTab}","'+editWindowID_zdy+'","&nbsp;'+buttonrules.tabstitle+'","editPage_${listPageRander}_zdy") />&nbsp;&nbsp;&nbsp;&nbsp;';
						...
						title="${buttonname}" sytle="cursor:pointer;"
						src="<%=request.getContextPath()%>/${buttonicon}"
						onclick=OpenWindowPage( "initdiv_${listPage.id}_listpagezdy","'+buttonrules.tabstitle+'","init_${listPage.id}_listpagezdy","'+LISTPAGE_LOAD+buttonrules.tabspageurl+params+'","380") />&nbsp;&nbsp;&nbsp;&nbsp;';
						...
						title="${buttonname}" sytle="cursor:pointer;"
						src="<%=request.getContextPath()%>/${buttonicon}"
						onclick=loadViewPage( "'+buttonrules.tabspageurl+'","'+loadEditUrl_JS+'","1","${idkey}","'+editPageDataDiv+'","${editPage.isUseTab}","'+editWindowID+'","&nbsp;'+buttonrules.tabstitle+'","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
											 	  }
