		...
		<td colspan="3" style="border-bottom:0px solid white;">
			 <c:choose>
		    	<c:when test="${textColumn.readOnly}">
		    	 	<textarea style="margin:0;width:552px;color:#565656;" id="editColumn_${stauts.index}${columnID}${editPage.id}" name="${name}" readonly=""  >${data}</textarea>
				</c:when>
		    	<c:otherwise>
		    		 <textarea style="margin:0;width:552px;color:#565656;" id="editColumn_${stauts.index}${columnID}${editPage.id}" name="${name}" >${data}</textarea>
				</c:otherwise>
			 </c:choose>
			   <c:choose>
 ...
 <!-- *****************************编辑列字段********************************** -->
		 ...
		 <c:choose>
				    	...
				    	<c:when test="${textColumn.readOnly}">
				    	 	<c:choose>
								 <c:when test="${textColumn.exclusiveLine}">
									...
									<input style="width:${editMode.tdWidth}px;" id="editColumn_${stauts.index}${columnID}${editPage.id}" type="text" name="${name}"   value="${data}"  readonly="" />
				    	 ...
				    	 <c:otherwise>
				    		 <c:choose>
								 <c:when test="${textColumn.exclusiveLine}">
										<input style="width:${editMode.tdWidth}px;" id="editColumn_${stauts.index}${columnID}${editPage.id}" type="text" name="${name}"   value="${data}" />
		   						 </c:when>
								 <c:otherwise>
										<input style="width:${editMode.tdWidth}px;" id="editColumn_${stauts.index}${columnID}${editPage.id}" type="text" name="${name}"   value="${data}" />
			   				      </c:otherwise>
							...
							<select class="easyui-combobox" id="editColumn_${stauts.index}${columnID}${editPage.id}" name='${name}' style='width:493px;' >
						 			<c:choose>
										<c:when test="${data eq '0' }">
											<option value="${data}" selected="selected" >请选择</option>
										</c:when>
				...
				<c:when test="${type==4}"><!--日历控件 -->
				    <input sid="${name}" value="${data}" required="${editMode.required}" id="editColumn_${stauts.index}${columnID}${editPage.id}"  type="text" name="${name}" class="easyui-datebox" readonly="readonly" style="width:150px;" />
					  <script>
						   $(function(){
    			...
    			<c:when test="${type==16}"><!-- 时间控件（时分秒） -->
					<c:choose>
						...
						<c:otherwise>
							<!-- easyui datebox -->
							 <input value="${data}" sid="${name}" required="${editMode.required}" id="editColumn_${stauts.index}${columnID}${editPage.id}"   name="${name}"  readonly="readonly"  />
							  <script>
								   $(function(){
								       $('#editColumn_${stauts.index}${columnID}${editPage.id}').datebox({
								       		editable:false,
    			...
    			<c:otherwise>
					<c:choose>
						 <c:when test="${textColumn.exclusiveLine}">
							 <c:choose>
						    	<c:when test="${textColumn.readOnly}">
						    		<input style="width:${editMode.textWidth}px;" id="editColumn_${stauts.index}${columnID}${editPage.id}" type="text" name="${name}"  class="easyui-validatebox" value="${data}" readOnly="readonly" />
								</c:when>
						    	<c:otherwise>
						    		<input style="width:${editMode.textWidth}px;" id="editColumn_${stauts.index}${columnID}${editPage.id}" type="text" name="${name}"  class="easyui-validatebox" value="${data}" />
								</c:otherwise>
						     </c:choose>
						 </c:when>
						 <c:otherwise>
