						...
						,{field:'col4',title:'操作',width:100,rowspan:2,align:'center',	
							formatter:function(value,rowData,rowIndex){
								...
								<c:choose>
									...
									<c:otherwise>
									   ...
									   var precursorId=rowData.eng_envstatus+'';//前驱状态ID
										...
										<%  
										if(authority!=null){
											if(authority.get("'"+listPage.getId()+"_edit'")!=null&&(!"all".equals(workflowFiter))){
										%>
											if(precursorId==-1){
												opertorString='<img  style="cursor:hand;" title="编辑" sytle="cursor:pointer;"  width="12px" src="<%= request.getContextPath()%>/images/ioc-editor.gif" onclick=loadEditPage_easyui("${listPage.id}","${isAbleWorkFlow}","'+editFormID+'","'+loadEditUrl+'","1","${idkey}","'+editWindowID+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;${editPage.title}","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
											}else{
								...
								<c:choose>
									...
									<%  
									  if(authority!=null){
										if(authority.get("'"+listPage.getId()+"_delete'")!=null&&(!("all".equals(workflowFiter)|("daibanxiang".equals(workflowFiter))))){
									%>
					...
					<c:otherwise>
						...
						%>
										
							{
								...
								iconCls:'icon-add',
								handler:function(){
									loadEditPage_easyui("${listPage.id}","${isAbleWorkFlow}",""+editFormID,"","0","${idkey}",""+editWindowID,"${listPage.editPage.isUseTab}",""+editWindowID,"&nbsp;${listPage.editPage.title}","editPage_${listPageRander}","${listPage.parentAppId}");
