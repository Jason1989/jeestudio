...
function initActivityTab(){
  $('#activity-tab-window').window({
		  ...
		  closed: true,
		  onClose:function(){
	          easyuiWinClose('activity-tab-window');
	      }
  });
}
/**
								...
								<c:choose>
									...
									<c:otherwise>
									   ...
									   var precursorId=rowData.eng_envstatus+'';//前驱状态ID
										...
										%>
											if(precursorId==-1){
												opertorString='<img  style="cursor:hand;" title="编辑" sytle="cursor:pointer;"  width="12px" src="<%= request.getContextPath()%>/images/ioc-editor.gif" onclick=loadEditPage_easyui("'+editFormID+'","'+loadEditUrl+'","1","${idkey}","'+editPageDataDiv+'","${listPage.editPage.isUseTab}","'+editWindowID+'","&nbsp;${editPage.title}","editPage_${listPageRander}") />&nbsp;&nbsp;&nbsp;&nbsp;';
											}else{
