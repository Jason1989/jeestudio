...
<form method="post" id="queryPage_${listPage.id}" action="${gridUrl}">
	...
	<input type="hidden" name="formID" value="${listPage.id}" />
	<!-- listPage id -->
				...
				<s:iterator value="#queryZone.queryColumns" id="queryZone" status="status">
					...
					<td>
						<c:choose>
							<c:when test="${type==1}">
								<!-- 文本框 -->
								<input id="queryColumn_${queryColumnID}_${status.index}"
							...
							<c:when test="${type==0}">
								<!-- 复选框 -->
								<input id="check_" type="checkbox" name="${name}"
									...
									<c:forEach items="${dictionaryData}" var="dictionaryDataMap">
										<c:choose>
											<c:when test="${dictionaryDataMap.key ==data}">
												<option value="${dictionaryDataMap.key}" selected="selected">
													${dictionaryDataMap.value}
												</option>
											</c:when>
											<c:otherwise>
												<option value="${dictionaryDataMap.key}">
													${dictionaryDataMap.value}
												</option>
											</c:otherwise>
										</c:choose>
							...
							<c:when test="${type==4}">
								<!-- 日历控件 -->
								<input value="${data}" id="datebox_${stauts.index}" type="text"
									name="${name}" class="easyui-datebox" />
								<script>
												   $(function(){
											  ...
											  &nbsp;&nbsp;<img title="清除"
									src="<%=request.getContextPath()%>/images/ioc-delete2.gif"
									onclick="clearText('datebox_${stauts.index}_${name}','null');"
									style="cursor: hand" />
