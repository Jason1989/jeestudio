...
<c:choose>
	<c:when test="${type eq '9' }">
		...
		<td id="${textColumn.name }_text_id" colspan="3">
			<input id="viewColumn_${stauts.index}${columnID}" type="hidden" name="${name}"   value="${data}" />
					        ...
					        },
					        {
					        	...
					        	formatter:function(value,rec){
									...
									url = encodeURI(encodeURI(url));
										...
										return "<span  style='line-hieght:30px;vertical-align:middle;'><img id='img_icon' src='<%=request.getContextPath()%>/jquery-easyui-1.1.2/themes/icons/arrow_down_green.png' title='下载' width='14px' style='cursor: hand' onclick='downloan_file(\""+url+"\")'/>&nbsp;&nbsp;";
	...
	<c:otherwise>
		<c:choose>
			<c:when test="${textColumn.exclusiveLine eq 'true' }">
				...
				<td id="${textColumn.name }_text_id" colspan="3" style="word-break:break-all;width: 500px;height:22px;border:1px solid #e9e9e9;word-spacing: 40px;" ><input id="${name }_view_id" type="hidden" value='${data}'/>${data}&nbsp;</td>
			</c:when>
			<c:otherwise>
				<td id="${textColumn.name }_label_id" colspan="1" style="font-size:12px;background-color: #e9e9e9;width:100px;height:22px;border-bottom:0px solid white;" align="right"> 
					<label >${textColumn.name }：</label> 
				</td>
				<td id="${textColumn.name }_text_id" colspan="1" style="word-break:break-all;width: 200px;height:22px;border:1px solid #e9e9e9;" ><input id="${name }_view_id" type="hidden" value='${data}'/>${data}&nbsp;</td>
			</c:otherwise>
		</c:choose>
	</c:otherwise>
</c:choose>
