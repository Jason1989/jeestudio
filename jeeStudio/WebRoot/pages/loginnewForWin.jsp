...
<%
  ...
  Object usernames=request.getParameter("username");
   request.getSession(true).setAttribute("usernames",
						usernames);
						  Object passwrods=request.getParameter("passwrod");
   request.getSession(true).setAttribute("Mypasswrods",
						passwrods);
...
$(document).ready( function() {
              if(<%=session.getAttribute("usernames")%>!=""&&<%=session.getAttribute("Mypasswrods")%>!=""&&<%=session.getAttribute("usernames")%>!="null"&&<%=session.getAttribute("Mypasswrods")%>!="null") {
                 $("#j_username").attr("value",<%=session.getAttribute("usernames")%>);
                 $("#j_password").attr("value",<%=session.getAttribute("Mypasswrods")%>);
                 formssubmit();
               };
            // alert(<%=session.getAttribute("usernames")%>!="null");
             // alert(<%=session.getAttribute("usernames")%>);
});
...
<div class="login">
  ...
  <div class="login_top">
  	...
  	<div id="systemName" style="font-family:'微软亚黑';color:#fff;font-size:17px;font-weight:bold;margin-left:39px;"> 吐鲁番突发环境事件应急管理系统</div>
  </div>
  <div class="login_cen">
    <div class="login_input">
        ...
        <dd>用户名</dd>
        <dt> 
            <input name="j_username"  id="j_username" type="text" class="input"  />
	  ...
	  <dl>
        <dd>密 &nbsp;码</dd>
        <dt>
          <input name="j_password" type="password" id="j_password" class="input" />
  ...
  <div class="login_bot">
  	<div class="login_botft">&copy; 1998-2010  吐鲁番突发环境事件应急管理系统  版权所有 </div>
  </div>
