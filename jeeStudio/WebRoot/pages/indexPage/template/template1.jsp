				...
				<div region="west" border="false" style="width:${clientWidth*0.30}px;" class="index_model_padding">
						<c:choose>
							<c:when test="${not empty divmap['node3'].name}">
								<div class="easyui-panel CM_20_01" border="true" fit="true" href="${divmap['node3'].url}" id="node3"  title="${divmap['node3'].name}"></div>
							</c:when>
							<c:otherwise>
								<div class="easyui-panel CM_20_01" border="true" fit="true" href="${url}" id="node3"  title="${title}"></div>
							</c:otherwise>
						</c:choose>
				</div>
				...
				<div region="center" border="false"  class="index_model_padding">
					<c:choose>
						<c:when test="${not empty divmap['node4'].name}">
							<div class="easyui-panel CM_20_02" border="true" fit="true" href="${divmap['node4'].url}" id="node4"  title="${divmap['node4'].name}"></div>
						</c:when>
						<c:otherwise>
							<div class="easyui-panel CM_20_02" border="true" fit="true" href="${url}" id="node4"  title="${title}"></div>
						</c:otherwise>
					</c:choose>
				</div>
					...
					<div region="west" border="false" style="width:${clientWidth*0.25}px;" class="index_model_padding">
						<c:choose>
							<c:when test="${not empty divmap['node5'].name}">
								<div class="easyui-panel CM_30_01" border="true" fit="true" href="${divmap['node5'].url}" id="node5"  title="${divmap['node5'].name}"></div>
							</c:when>
							<c:otherwise>
								<div class="easyui-panel CM_30_01" border="true" fit="true" href="${url}" id="node5"  title="${title}"></div>
							</c:otherwise>
						</c:choose>
					</div>
			    <!-- center必须显示  -->
				<div region="center" border="false"  class="index_model_padding">
					<c:choose>
						<c:when test="${not empty divmap['node6'].name}">
							<div class="easyui-panel CM_30_02" border="true" fit="true" href="${divmap['node6'].url}" id="node6"  title="${divmap['node6'].name}"></div>
						</c:when>
						<c:otherwise>
							<div class="easyui-panel CM_30_02" border="true" fit="true" href="${url}" id="node6"  title="${title}"></div>
						</c:otherwise>
					</c:choose>
				</div>
			    <div region="east" border="false" style="width: ${clientWidth*0.25}px;" class="index_model_padding">
			    	<c:choose>
						<c:when test="${not empty divmap['node7'].name}">
							<div class="easyui-panel CM_30_03" border="true" fit="true" href="${divmap['node7'].url}" id="node7"  title="${divmap['node7'].name}"></div>
						</c:when>
						<c:otherwise>
							<div class="easyui-panel CM_30_03" border="true" fit="true" href="${url}" id="node7"  title="${title}"></div>
						</c:otherwise>
					</c:choose>
			    </div>
			</div>
		...
		<div region="north" border="false" style="height:${clientHeight*0.25}px;">
			<div class="easyui-layout" fit="true" border="false">
				...
				<c:if test="${westflag eq false}">
					<div region="east" border="false"  class="index_model_padding" style="width:${clientWidth*0.45}px;">
					</div>
				</c:if>
			</div>		    
		</div>
