...
import com.zxt.compplatform.codegenerate.codeFactory.EngineCodeGenerateFactory;
...
public class EngineCodeGenerateServiceImpl implements IEngineCodeGenerateService {
	private ICodeGenerateDao codeGenerateDao;
	private PageService pageService;
	private static final Logger log = Logger
			.getLogger(EngineCodeGenerateServiceImpl.class);
	...
	public boolean saveGenerateCode(String packageName, String basePath,
		...
		try {
			...
			EngineCodeUseEntity engineCodeUseEntity = new EngineCodeUseEntity();
			engineCodeUseEntity.setBasePath(basePath);
			engineCodeUseEntity.setPackageName(packageName);
			engineCodeUseEntity.setBasePage(basePage);
			engineCodeUseEntity.setJarName(jarName);
			engineCodeUseEntity.setVersionId("V1." + versionId);
			// //视图层
			String viewDir = fileDir + File.separator + pagePath;
			createDir(viewDir);
			engineCodeUseEntity.setBasePage(basePage);
			engineCodeUseEntity.setJarName(jarName);
			engineCodeUseEntity.setClassName(jarName);
			engineCodeUseEntity.setOutPath(viewDir);
			engineCodeUseEntity.setCodeType(engineCodeUseEntity.CODE_GENERATE_TYPE_VIEW);
			EngineCodeGenerateFactory.getInstance(engineCodeUseEntity).engineCodeGenerate(
					engineCodeUseEntity);
			int i = 1;
			if (tabs.size() > 0) {
				...
				while (tabsIt.hasNext()) {
					...
					engineCodeUseEntity.setBasePage(base);
					engineCodeUseEntity.setJarName(jarName);
					engineCodeUseEntity.setClassName(jarName + "tab" + i);
					EngineCodeGenerateFactory.getInstance(engineCodeUseEntity)
							.engineCodeGenerate(engineCodeUseEntity);
					i++;
				}
			}
			//			
